...
stmts : expr TSEMICOLON {$$ = make_node(STATEMENT_LIST, $1, NULL, null_value(), type_void());}
      | stmts expr TSEMICOLON {$$ = make_node(STATEMENT_LIST, $2, $1, null_value(), type_void());}
      ;

control_structure : TIF expr expr {$$ = make_node(IF, $2, $3, null_value(), type_void());}
		  | TWHILE expr expr {$$ = make_node(WHILE, $2, $3, null_value(), type_void());}
		  ;
...
func_decl : TDEF TIDENTIFIER TLPAREN ident_list TRPAREN TARROW type expr {
...
expr : TIDENTIFIER {$$ = make_node(GET_SCALAR, $1, NULL, null_value(), type_void());}
     ...
     | external_decl {$$ = $1;}
     | control_structure {$$ = $1;}
     | TRETURN expr {$$ = make_node(RETURN, $2, NULL, null_value(), type_void());}
